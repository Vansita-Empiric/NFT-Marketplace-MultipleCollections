...
contract AuctionNFT {
    ...
    mapping (SimpleNFT => address[]) mintedNFTs;
    ...
    // SimpleNFT array to store NFT Collection address
    SimpleNFT[] nftCollectionAddresses;
    ...
    function mintNFTCollection(string memory name, string memory symbol) isUserLoggedIn external {
        SimpleNFT nftCollectionAddress = new SimpleNFT(name, symbol);
        nftCollectionAddresses.push(nftCollectionAddress);
    }

    // To mint NFT in specified collection
    function mintNFT(SimpleNFT collectionAddress) isUserLoggedIn external {
        collectionAddress.createNFT(msg.sender);
        mintedNFTs[collectionAddress].push(msg.sender);
    }

    // To get all minters of specific NFTCollection's NFT
    function getMinters(SimpleNFT collectionAddress) isUserLoggedIn external view returns (address[] memory) {
    ...
    function auctionNFT(
        ...
        SimpleNFT nftCollection = SimpleNFT(_collectionAdd);
    ...
    // Ending Auction
    function auctionEnd(bytes4 _aId, SimpleNFT _collectionAdd) external {
        Auction storage auction = auctions[_aId];
        // Check if the auction is active
        if (!isActive[_aId]) {
            revert CustomErrorWithReason("Auction is not active");
    ...
    function showNFTCollections() public view returns (SimpleNFT[] memory) {
